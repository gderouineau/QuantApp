...
class Command(BaseCommand):
    help = (
        "Train MLP avec normalisation pos_* par y_resolution et MIRROR si is_long=False (pos := 1 - pos). "
        "Cible sélectionnable: --target tp1|tp2|tp3|any|auto. Gating optionnel is_entry/is_setup."
    )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--gate_entries", action="store_true")
        parser.add_argument("--gate_setups", action="store_true")
    ...
    def handle(self, *args, **opts):
        ...
        if not ds.dataset_path or not ds.labels_path:
            raise CommandError("Missing dataset/labels parquet. Run ns_dataset_indicators ET ns_labels_everybar d'abord.")

        if opts["gate_entries"] and opts["gate_setups"]:
            raise CommandError("Choisir au plus un gating: --gate_entries OU --gate_setups.")

        run = NSRun.objects.create(dataset=ds, kind="train_mirror", status="pending")
        run.mark_running()
        ...
        try:
            ...
            # 2) choix de la cible
            has_tp1 = "y_tp1" in tbl.columns
            has_tp2 = "y_tp2" in tbl.columns
            has_tp3 = "y_tp3" in tbl.columns
            if target in ("tp1", "tp_1") and has_tp1:
                tbl["y"] = tbl["y_tp1"].astype(int)
            elif target in ("tp2", "tp_2") and has_tp2:
                tbl["y"] = tbl["y_tp2"].astype(int)
            elif target in ("tp3", "tp_3") and has_tp3:
                tbl["y"] = tbl["y_tp3"].astype(int)
            elif target in ("any", "tp_any"):
                cols = [c for c in ("y_tp1", "y_tp2", "y_tp3") if c in tbl.columns]
                if cols:
                    tbl["y"] = (tbl[cols].max(axis=1) >= 1).astype(int)
            else:
                # auto ou fallback : 'y' déjà présent
                tbl["y"] = tbl["y"].astype(int)

            # 3) normalisation pos_*
            y_res = int(ds.y_resolution or 0)
            pos_cols = [c for c in feat_cols if c.startswith("pos_")]
            if y_res > 0 and pos_cols:
                tbl[pos_cols] = (tbl[pos_cols].apply(pd.to_numeric, errors="coerce").fillna(0.0) / float(y_res))

            # 4) mirroring si is_long == False
            ...
            # 6) garde-fou 'y' (si doublon résiduel pour une quelconque raison)
            if isinstance(tbl.get("y"), pd.DataFrame):
                tbl["y"] = tbl["y"].iloc[:, 0].astype(int)

            if tbl.empty or int(tbl["y"].sum()) == 0:
                raise CommandError("Après jointure/gating, aucune cible positive. Ajuste horizon/stop/target.")

            # 7) split temporel
            train_mask, val_mask = split_masks_by_timeframe(pd.to_datetime(tbl.index, utc=True), ds.timeframe)
            train_df = tbl[train_mask].dropna(subset=["y"])
            val_df = tbl[val_mask].dropna(subset=["y"])
            if len(train_df) == 0 or len(val_df) == 0:
                raise CommandError("Split train/val vide. Vérifie dates/timeframe.")
            ...
            for epoch in range(int(opts["epochs"])):
                ...
                if val_prec > best_val_prec:
                    ...
                    best_state = {
                        ...
                        "curve": curve[:5],
                        "target": target,
                        "y_resolution": int(ds.y_resolution or 0),
                    }
                    bad = 0
                else:
                    bad += 1
            ...
            out_path = os.path.join(out_dir, f"ds{ds.id}_tp{int(tp_margin*100)}_mirror.pt")
